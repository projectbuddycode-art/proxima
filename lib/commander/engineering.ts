...
export class RealDevelopmentCommanderEngine {
  ...
  static async auditSystemState(): Promise<{
    ...
    const conn = (await ai.testConnection().catch(() => ({ ok: false, models: [] as string[], message: 'Offline' }))) as { ok: boolean; models?: string[]; message?: string };
    const ollamaStatus = conn.ok ? `ONLINE (${conn.models?.join(', ') || 'ready'})` : 'OFFLINE';
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        tasks = rows.map((r: any) => ({
          ...
          test_results: typeof r.tests_json === 'string' ? JSON.parse(r.tests_json) : (r.tests_json || { passed: 0, total: 0, output: 'Awaiting execution' }),
