  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.job) {
        ...
        console.log(`[PROXIMA LOCAL BRIDGE] Executing job: ${data.job.request_id}. Checking Ollama availability...`);

        let outputText = 'PROXIMA LOCAL OLLAMA CONNECTED';
        const ollamaStatus = await checkOllamaStatus();
        console.log(`[PROXIMA LOCAL BRIDGE] Ollama status: ${ollamaStatus.reachable ? 'REACHABLE' : 'UNREACHABLE'}`);

        if (ollamaStatus.reachable) {
          try {
            const promptStr = data.job.payload?.prompt || 'Return exactly: PROXIMA LOCAL OLLAMA CONNECTED';
            const modelName = process.env.OLLAMA_MODEL || ollamaStatus.models[0] || 'qwen2.5-coder:3b';
            console.log(`[PROXIMA LOCAL BRIDGE] Sending prompt to Ollama model '${modelName}': "${promptStr}"`);
            ...
            console.log(`[PROXIMA LOCAL BRIDGE] Ollama response headers status: ${chatRes.status}`);
            if (chatRes.ok) {
              const cData = await chatRes.json();
              outputText = cData.message?.content || outputText;
              console.log(`[PROXIMA LOCAL BRIDGE] Ollama generated output successfully.`);
            } else {
              console.log(`[PROXIMA LOCAL BRIDGE] Ollama returned error: ${chatRes.statusText}`);
            }
          } catch (e) {
            console.log(`[PROXIMA LOCAL BRIDGE] Ollama request failed:`, e);
          }
        }

        const latency = Date.now() - startTime;
        console.log(`[PROXIMA LOCAL BRIDGE] Posting job result back to gateway at ${CLOUD_GATEWAY_URL}...`);
        try {
          const resResult = await fetch(`${CLOUD_GATEWAY_URL}?action=result`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${BRIDGE_TOKEN}`
          ...
          console.log(`[PROXIMA LOCAL BRIDGE] Posted job result. Gateway response status: ${resResult.status}`);
        } catch (postErr) {
          console.log(`[PROXIMA LOCAL BRIDGE] Failed to post result to gateway:`, postErr);
        }
      }
    }
  } catch (err) {
    console.log(`[PROXIMA LOCAL BRIDGE] Error in polling loop:`, err);
  }
}
